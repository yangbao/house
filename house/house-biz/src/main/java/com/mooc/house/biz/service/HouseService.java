...
public class HouseService {
	...
	/**
	 * 1.查询小区, 经常用到的查询: 通过小区名字--->小区ID--->得到房产 
	 * 2.添加图片服务器地址前缀
	 * 3.构建分页结果
	 * @param query - 房屋的查询条件, 不是完善的信息.
	 * @param pageParams - 分页的信息
	 * return PageData --分页的结果
//	 */
	public PageData<House> queryHouse(House query, PageParams pageParams) {
		
		List<House> houses = Lists.newArrayList();
		//看是否房产的名字是小区的名字,如果有则把相应的信息放到房产里面
	...
	public void updateRating(Long id, Double rating) {
		...
		Double newRating  = oldRating.equals(0D)? rating : Math.min((oldRating+rating)/2, 5);// 别超过5分, 怕是被注入的
